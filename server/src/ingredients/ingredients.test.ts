...
import { Ingredient, Ingredients } from './index';

describe('Ingredient', () => {
  async function getRecipes() {
    const { recipeDb } = await createTestDbs();
    return recipeDb.getRecipes(undefined, 'ah', 'john doe');
  }
  ...
  it('gets packed quantity', () => {
    const i = new Ingredient('3 Aardappels');
    expect(i.quantityToOrder()).toBe(1);
  });

  it('150 ml pot should result in 1 unit to order', () => {
    const ing = new Ingredient('150 ml pot natural low-fat yogurt');
    expect(ing.quantityToOrder()).toBe(1);
  });

  it('gets right quantity', () => {
    let ing = new Ingredient('2 x 460g jars roasted red peppers');
    expect(ing.quantityToOrder()).toBe(2);
    ing = new Ingredient('2x400 g organic tinned plum tomatoes');
    expect(ing.quantityToOrder()).toBe(2);
    ing = new Ingredient('2 blikken (à 400 ml) tomaatstukjes');
    expect(ing.quantityToOrder()).toBe(2);
    ing = new Ingredient('2 tbsp tomato purée');
    expect(ing.quantityToOrder()).toBe(1);
  });

  it('ingredient with comma', () => {
    equals('200g gehalveerde, gewelde pruimen zonder pit', [
      200,
      'g',
      'pruimen zonder pit',
    ]);
  ...
  it('returns amount of cans (packed)', () => {
    const i = new Ingredient('3 cans of tomatoes');
    expect(i.quantityToOrder()).toBe(3);
  });
  ...
  it('first ingredient then quantity', () => {
    equals('2 garlic cloves, crushed', [2, 'cloves', 'garlic']);
  });

  it('has 2 cloves in ingredient', () => {
    equals('2 cloves garlic', [2, 'cloves', 'garlic']);
  });
  ...
  it('validations from recipes from db.unit-test.json', () => {
    ...
    //   [2, "handfuls of a mixed pack", "of sweetcorn"]
    // )
    equals('0,5 lt vleesbouillon (pot of tablet)', [
      '0,5',
      'lt',
      'vleesbouillon',
    ]);
    // equals("salade voor Gigantes plaki", [
    //   undefined,
    //   undefined,
    //   "salade voor Gigantes plaki",
    // ])
    equals('2 heaped tbsp tikka masala paste', [
      2,
      'heaped tbsp',
      'tikka masala paste',
    ]);

    equals('200 g gare kastanjes (pot of vacuüm) ', [
      200,
      'g',
      'gare kastanjes',
    ]);
    equals(
      '720 ml passata di pomodoro (fles à 720 ml, AH puur en eerlijk biologisch)',
      [720, 'ml', 'passata di pomodoro'],
    );
    // strip is also english unit
    equals('grote braadzak met sluitstrip', [
      undefined,
      undefined,
      'grote braadzak met sluitstrip',
    ]);
    equals('2 citroenen in plakjes gesneden', [2, 'plakjes', 'citroenen in']);
    equals('2 laurierblaadjes', [2, undefined, 'laurierblaadjes']);
    ...
    equals('170 g pack smoked salmon', [
      undefined,
      '170 g pack',
      'smoked salmon',
    ]);
    ...
    equals('400g bag mixed seafood', [undefined, '400 g bag', 'mixed seafood']);
    ...
    equals('1/2 teaspoon(s) baking soda', [
      '1/2',
      'teaspoon(s)',
      'baking soda',
    ]);
    ...
    equals('2 x 140g skinless hake fillets', [2, '140g', 'hake fillets']);
    ...
    equals('500g gepelde, gehalveerde sjalotjes', [500, 'g', 'sjalotjes']);
  ...
  it('all ingredients from Paprika can be parsed', async () => {
    ...
    // const fs = require("fs")
    // fs.writeFileSync("Output.json", JSON.stringify(parsedIngredients))
    // return
    ...
    equalsAll(parsedIngredients, [
      ...
      [
        ...
        [undefined, '200 g pack', 'green vegetable medley'],
      ...
      [
        ...
        [85, 'g', 'butter'],
        [undefined, '170 g pot', '0% fat Greek yogurt'],
        [850, 'ml', 'semi-skimmed milk'],
        [75, 'g', 'plain flour'],
        [2, 'tsp', 'English mustard'],
        [1, 'tsp', 'wholegrain mustard'],
        [undefined, '300 g pack', 'mature cheddar'],
      ...
      [
        ...
        [1, undefined, 'onion'],
        [undefined, '260 g bag', 'spinach'],
        [2, '140g', 'hake fillets'],
        ['½', 'tsp', 'sweet smoked paprika'],
        [1, undefined, 'red chilli'],
        [undefined, '400 g can', 'cannellini beans'],
      ...
      [
        ...
        [2, '400g cans', 'chopped tomatoes'],
        [1, 'tbsp', 'clear honey'],
        [undefined, '500 g pack', 'fresh egg lasagne sheets'],
