...
describe('storeRecipe()', () => {
  ...
  beforeAll(async () => {
    ({ orderDb, recipeDb } = await createTestDbs());
  });
  ...
  it('get hydrated', async () => {
    ...
    const hydrated = orders[0];
    expect(hydrated.date).toBeDefined();
    expect(hydrated.recipes.length).toBe(1);
    const recipe = hydrated.recipes[0];
    ...
    await orderDb.remove({ date: hydrated.date });
  });
  ...
  it('store order', async () => {
    ...
    const hydrated = orders[0];
    expect(hydrated.recipes.length).toEqual(1);
    const recipe = hydrated.recipes[0];
    expect(recipe.parsedIngredients).toMatchSnapshot();

    await orderDb.remove({ uid: hydrated.uid });
  });
});
